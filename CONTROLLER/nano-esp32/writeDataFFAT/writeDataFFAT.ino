#include "FS.h"
#include "FFat.h"  // Utilisation de FATFS sur ESP32-S3

// Déclaration de la variable contenant du HTML (ou tout autre texte)
const char PAGE_DBL[] = R"=====(bpm=120
2773;580;o-p-
2773;2192;o-p-
2773;577;o-p-
2765;137;o-\-
  ...
  <div id="tempo-container">
    ...
    <script>
        ...
        function updateCompilation() {
            ...
            const measures = ($('#grid td').length)/4-1;
...
void setup() {
    ...
    // Initialiser FFAT
    if (!FFat.begin(true)) {
        Serial.println("Erreur de montage FFAT !");
        return;
    }

    Serial.println("FFAT monté avec succès !");
...
//****************************************************
    filename = "/index.html";    
    File file2 = FFat.open(filename, "w");
    if (!file2) {
        Serial.println("Erreur d'ouverture du fichier.");
        return;
    }
    file2.print(PAGE_MAIN);  
    file2.close();

    Serial.println("Fichier index.html écrit avec succès !");

//****************************************************
    filename = "/zepto.min.js";    
    File file3 = FFat.open(filename, "w");
    if (!file3) {
        Serial.println("Erreur d'ouverture du fichier.");
        return;
    }
    file3.print(PAGE_JQUERY);  
    file3.close();


    Serial.println("Fichier zepto.min.js écrit avec succès !");
}

void loop() {
    // Pas de code dans loop() pour cet exemple
}
